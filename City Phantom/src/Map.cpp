    ...
    if (mapLoaded) {
        ...
        for (const auto& mapLayer : mapData.layers) {
            ...
            if (mapLayer->properties.GetProperty("Draw") != NULL && mapLayer->properties.GetProperty("Draw")->value == true) {
				...
				if (mapLayer->name != "Building" && mapLayer->name != "Boss")
				{
                ...
                else if (mapLayer->name == "Boss" && BossRoom == true)
                {
                    for (int i = 0; i < mapData.width; i++) {
                        for (int j = 0; j < mapData.height; j++) {
...
bool Map::Load(std::string path, std::string fileName)
{
    ...
    if(result == NULL)
	{
        ...
        for (const auto& ObjectGroups : mapData.objectgroups) {
            if (ObjectGroups->properties.GetProperty("Deadly") != NULL && ObjectGroups->properties.GetProperty("Deadly")->value == true) {
                for (const auto& Object : ObjectGroups->objects)
                {
                    PhysBody* collider = Engine::GetInstance().physics.get()->CreateRectangle(Object.x + Object.width / 2, Object.y + Object.height / 2, Object.width, Object.height, STATIC);
                    collider->ctype = ColliderType::DEATH;
                }
...
MapLayer* Map::GetBuildingLayer()
{
    for (const auto& layer : mapData.layers) {
        if (layer->properties.GetProperty("Building") != NULL &&
            layer->properties.GetProperty("Building")->value) {
            return layer;
        }
    }
